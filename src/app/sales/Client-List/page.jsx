...
// Normalise a product name for dedup: trim + lowercase
const normalise = (s = "") => s.trim().toLowerCase();

/* ══════════════════════════════════════════
   STAT CARD  (original design preserved)
══════════════════════════════════════════ */
...
export default function ClientsPage() {
  ...
  /* ── aggregate clients ──
     Products are stored normalised (trimmed, original casing preserved via
     a canonical map so "Head" and "head" collapse into one entry).
  ── */
    ...
    orders.forEach((o) => {
      ...
      if (!map[clientId]) {
        map[clientId] = {
          id: clientId,
          name: clientName,
          // key = normalised, value = canonical display name (first seen)
          productMap: {},
          totalOrders: 0,
          totalQty: 0,
          lastOrderDate: null,
      ...
      // Deduplicate products case-insensitively
  ...
  /* ── deduplicated product list for filter pills ──
     Collapse "Head", "head", "HEAD" → one pill
  ── */
  const allProducts = useMemo(() => {
    ...
    clients.forEach((c) =>
      c.products.forEach((p) => {
        const k = normalise(p);
        if (!seen[k]) seen[k] = p; // keep first-seen casing
      })
    );
  ...
  const filtered = useMemo(() => {
    ...
    return r;
  }, [clients, search, selectedProduct, sortField, sortDir]);

  /* ── reset page whenever filters change ── */
  useEffect(() => { setPage(1); }, [search, selectedProduct, sortField, sortDir]);
  ...
  const topClient   = [...filtered].sort((a, b) => b.totalOrders - a.totalOrders)[0];
      ...
      <div className="flex-1 overflow-auto">
        ...
        <div className="p-4 sm:p-8 space-y-6 sm:space-y-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-6">
            ...
            <StatCard
              title="Most Active"
              value={topClient ? topClient.totalOrders : "—"}
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-4 sm:p-6 shadow-sm space-y-4">
            ...
            {/* Product filter pills — deduplicated */}
            <div>
            ...
            {(search || selectedProduct !== "ALL") && (
              <div className="flex flex-wrap items-center gap-2 pt-1">
                ...
                <button
                  onClick={() => { setSearch(""); setSelectedProduct("ALL"); }}
