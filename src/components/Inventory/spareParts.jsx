...
const exportToCSV = (groupedParts, items) => {
  const rows = [];
  rows.push(["Part Name", "Vendor", "Location", "Quantity", "Max Quantity", "Status", "Remark"]);
...
export default function SparePartsInventory() {
  ...
  // Server-side pagination (for API calls)
  const [serverPage, setServerPage] = useState(1);
  const [serverTotalPages, setServerTotalPages] = useState(1);
  const SERVER_LIMIT = 100; // fetch 100 at a time, then paginate client-side
  ...
  const role = typeof window !== "undefined" ? JSON.parse(localStorage.getItem("user"))?.role : null;

  /* FORM */
  const [showForm, setShowForm] = useState(false);
  const [editId, setEditId] = useState(null);
  const [form, setForm] = useState({ partName: "", vendor: "", remark: "", location: "", quantity: "", maxQuantity: "" });
  ...
  useEffect(() => {
    axios.get(`${API}/vendors`, { headers }).then((res) => setVendors(res.data)).catch(console.error);
  }, []);

  // Reset client page when filters/search change
  useEffect(() => { setCurrentPage(1); }, [search, statusFilter, pageSize]);

  /* ================= SAVE ================= */
  const submitPart = async () => {
    try {
      if (!form.partName || !form.vendor || !form.location || form.quantity === "") return alert("All fields required");
      const payload = {
        partName: form.partName, vendor: form.vendor, remark: form.remark?.trim() || "",
        location: form.location, quantity: Number(form.quantity),
      };
      if (role === "admin" && form.maxQuantity !== "") payload.maxQuantity = Number(form.maxQuantity);
      if (editId) {
      ...
      setShowForm(false);
      setForm({ partName: "", vendor: "", location: "", remark: "", quantity: "", maxQuantity: "" });
      setEditId(null);
      fetchParts();
  ...
  const submitTransfer = async () => {
    try {
      if (!transfer.toLocation || transfer.quantity === "") return alert("Destination location and quantity required");
  ...
  const groupedParts = useMemo(() => {
    ...
    items.forEach((item) => {
      ...
      if (!grouped.has(normalizedKey)) {
        grouped.set(normalizedKey, { partName: item.partName, locations: [], totalQuantity: 0, totalMaxQuantity: 0, worstStatus: "Healthy", locationCount: 0 });
      }
      const group = grouped.get(normalizedKey);
      group.locations.push({
        id: item._id, location: item.location, quantity: item.quantity, maxQuantity: item.maxQuantity,
        status: item.status, remark: item.remark || "", vendorName: item.vendor?.name || "—", vendorRole: formatRole(item.createdByRole),
      });
      group.totalQuantity += item.quantity;
      group.totalMaxQuantity += item.maxQuantity || 0;
      group.locationCount = group.locations.length;
      const statusPriority = { Critical: 4, "Low Stock": 3, Overstocked: 2, Healthy: 1 };
      if (statusPriority[item.status] > statusPriority[group.worstStatus]) group.worstStatus = item.status;
    });
  ...
  const toggleExpand = (partName) => {
    ...
    newExpanded.has(partName) ? newExpanded.delete(partName) : newExpanded.add(partName);
    ...
    <div className="flex h-screen bg-gray-50 text-gray-900">
      <input
        ...
        onChange={async (e) => {
          ...
          try {
            await axios.post(`${API}/inventory/spare-parts/bulk-upload`, formData, { headers });
            fetchParts(); e.target.value = ""; alert("Bulk upload successful ✅");
          } catch (err) { console.error("Bulk upload failed", err); alert("Bulk upload failed ❌"); }
        }}
      />
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3 flex-1 min-w-0">
              ...
              <div className="min-w-0">
                <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900 truncate leading-tight">Spare Parts Inventory</h1>
                <p className="text-xs sm:text-sm text-gray-500 mt-0.5 hidden sm:block">Manage your spare parts stock levels and details</p>
              </div>
            </div>
        ...
        <div className="p-3 sm:p-5 md:p-6 lg:p-8 space-y-5 md:space-y-6">
          ...
          {lowStock > 0 && (
            <div className="bg-red-50 border border-red-200 rounded-xl p-3 sm:p-4 flex gap-2 sm:gap-3 items-center">
              ...
              <span className="text-red-700 font-medium text-xs sm:text-sm">{lowStock} part{lowStock !== 1 ? "s" : ""} need restocking</span>
            </div>
          )}
              ...
              <table className="w-full text-sm border-collapse">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    ...
                    <th className="px-4 py-3 text-right font-semibold text-gray-600 whitespace-nowrap">Total Stock</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan="9" className="py-16 text-center text-gray-400 text-sm">No parts found</td>
                    </tr>
                  ) : (
                    paginatedParts.map((part) => (
                      <React.Fragment key={part.partName}>
                        ...
                        <tr className="hover:bg-blue-50/30 cursor-pointer transition-colors group" onClick={() => toggleExpand(part.partName)}>
                          ...
                          <td className="px-4 py-3 text-gray-500 text-xs">{[...new Set(part.locations.map((l) => l.vendorName))].join(", ") || "—"}</td>
                        ...
                        {expandedParts.has(part.partName) && part.locations.map((loc, idx) => (
                          <tr key={loc.id} className={`bg-slate-50/70 transition-colors hover:bg-slate-100/60 ${idx === part.locations.length - 1 ? "border-b-2 border-gray-200" : ""}`}>
                            ...
                            <td className="px-4 py-2.5 text-xs text-gray-500">{loc.vendorName}</td>
                            <td className="px-4 py-2.5 text-xs text-gray-500 max-w-[160px] truncate" title={loc.remark}>{loc.remark || "—"}</td>
                            <td className="px-4 py-2.5">
                              ...
                              <div className="flex items-center gap-1">
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    setEditId(loc.id);
                                    setForm({ partName: part.partName, vendor: items.find((i) => i._id === loc.id)?.vendor?._id || "", location: loc.location, quantity: loc.quantity, maxQuantity: loc.maxQuantity ?? "", remark: items.find((i) => i._id === loc.id)?.remark || "" });
                                    setShowForm(true);
                                  }}
                                ...
                                <button onClick={(e) => { e.stopPropagation(); deletePart(loc.id); }} className="p-1.5 hover:bg-red-100 rounded-md transition" title="Delete">
                    ...
                    <div className="p-4 cursor-pointer" onClick={() => toggleExpand(part.partName)}>
                      <div className="flex items-start justify-between mb-2">
                        <div className="flex items-center gap-2 flex-1 min-w-0">
                          {expandedParts.has(part.partName) ? <ChevronDown size={15} className="text-gray-400 flex-shrink-0" /> : <ChevronRight size={15} className="text-gray-400 flex-shrink-0" />}
                      ...
                      <div className="border-t border-gray-200 divide-y divide-gray-100 bg-gray-50/50">
                        {part.locations.map((loc) => (
                          <div key={loc.id} className="p-4">
                            ...
                            {loc.remark && <p className="text-xs text-gray-400 italic mb-2">{loc.remark}</p>}
                            <div className="flex gap-2 mt-3">
                              ...
                              <button onClick={(e) => { e.stopPropagation(); deletePart(loc.id); }} className="bg-red-50 hover:bg-red-100 text-red-700 py-2 px-3 rounded-lg transition flex items-center justify-center">
                                <Trash2 size={13} />
                              </button>
                            </div>
                          </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-white p-5 sm:p-6 rounded-2xl w-full max-w-md border border-gray-200 shadow-xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={resetForm} className="text-gray-400 hover:text-gray-600 p-1 hover:bg-gray-100 rounded-lg transition"><X size={18} /></button>
            </div>
            <Input label="Spare Part Name" value={form.partName} onChange={(v) => setForm({ ...form, partName: v })} placeholder="Enter part name" />
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Vendor</label>
              <select value={form.vendor} onChange={(e) => setForm({ ...form, vendor: e.target.value })} className="w-full p-2.5 border border-gray-300 rounded-xl text-sm focus:border-[#c62d23] focus:ring-2 focus:ring-[#c62d23]/20 outline-none">
                <option value="">Select Vendor</option>
                {vendors.map((v) => <option key={v._id} value={v._id}>{v.name}</option>)}
              </select>
            </div>
            <Input label="Location" value={form.location} onChange={(v) => setForm({ ...form, location: v })} placeholder="Enter location" />
            <Input label="Remark" value={form.remark} onChange={(v) => setForm({ ...form, remark: v })} placeholder="Optional remark" />
            <Input label="Quantity" type="number" value={form.quantity} onChange={(v) => setForm({ ...form, quantity: v })} placeholder="Enter quantity" />
            {role === "admin" && <Input label="Max Quantity" type="number" value={form.maxQuantity} onChange={(v) => setForm({ ...form, maxQuantity: v })} placeholder="Enter max stock level" />}
            <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-100">
              <button onClick={resetForm} className="px-5 py-2.5 text-gray-700 font-medium rounded-xl border border-gray-300 hover:bg-gray-50 transition text-sm">Cancel</button>
              <button onClick={submitPart} className="bg-[#c62d23] hover:bg-[#a8241c] text-white px-5 py-2.5 font-medium rounded-xl transition shadow-sm hover:shadow-md text-sm">{editId ? "Update" : "Save"}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-white p-5 sm:p-6 rounded-2xl w-full max-w-md border border-gray-200 shadow-xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => { setShowTransfer(false); setTransfer({ toLocation: "", quantity: "" }); setTransferItem(null); }} className="text-gray-400 hover:text-gray-600 p-1 hover:bg-gray-100 rounded-lg transition"><X size={18} /></button>
            </div>
            ...
            <div className="mb-4">
              ...
              <select value={transfer.toLocation} onChange={(e) => setTransfer({ ...transfer, toLocation: e.target.value })} className="w-full p-2.5 border border-gray-300 rounded-xl text-sm focus:border-[#c62d23] focus:ring-2 focus:ring-[#c62d23]/20 outline-none">
                <option value="">Select Location</option>
                {locations.filter((loc) => loc !== transferItem.location).map((loc) => <option key={loc} value={loc}>{loc}</option>)}
              </select>
            </div>
            ...
            <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-100">
              <button onClick={() => { setShowTransfer(false); setTransfer({ toLocation: "", quantity: "" }); setTransferItem(null); }} className="px-5 py-2.5 text-gray-700 font-medium rounded-xl border border-gray-300 hover:bg-gray-50 transition text-sm">Cancel</button>
              <button onClick={submitTransfer} className="bg-[#c62d23] hover:bg-[#a8241c] text-white px-5 py-2.5 font-medium rounded-xl transition shadow-sm hover:shadow-md text-sm">Transfer</button>
            </div>
          </div>
        </div>
      )}
...
const KpiCard = ({ title, value, icon, danger = false, info = false, active = false, onClick }) => (
  ...
  >
    ...
    <p className={`text-2xl sm:text-3xl font-bold ${danger ? "text-red-600" : info ? "text-blue-600" : "text-gray-900"}`}>{value}</p>
  </div>
);

const StatusBadge = ({ status, size = "default" }) => {
  const map = { Healthy: "bg-green-100 text-green-800", "Low Stock": "bg-amber-100 text-amber-800", Critical: "bg-red-100 text-red-800", Overstocked: "bg-blue-100 text-blue-800" };
  const sizeClasses = size === "sm" ? "px-2 py-0.5 text-[10px]" : "px-2.5 py-1 text-[11px]";
  return (
    <span className={`${sizeClasses} rounded-full font-semibold whitespace-nowrap ${map[status] || "bg-gray-100 text-gray-700"}`}>{status}</span>
  );
};
