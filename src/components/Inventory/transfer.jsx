...
/* ─────────────────────────────────────────────────────────
   SEARCHABLE COMBOBOX  (replaces <select> in the modal)
───────────────────────────────────────────────────────── */
function SearchableCombobox({
  ...
  options,       // [{ label, value }]
  placeholder,
  allowNew,      // show "➕ Add …" row
  newLabel,      // e.g. "Add new location"
  onAddNew,      // () => void — open an inner mini-form
}) {
  ...
  return (
    <div ref={ref} className="relative">
      <div
        onClick={() => setOpen((o) => !o)}
        className="w-full flex items-center justify-between bg-white border border-gray-200 px-3 py-2.5 rounded-xl text-sm cursor-pointer focus-within:border-[#c62d23] focus-within:ring-2 focus-within:ring-[#c62d23]/15 transition-all"
      >
        {open ? (
          <input
            autoFocus
      ...
      {open && (
        <div className="absolute z-30 left-0 right-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-xl max-h-52 overflow-auto">
          {filtered.length === 0 && (
          ...
          {filtered.map((opt) => (
            <div
              ...
              className={`px-4 py-2.5 text-sm cursor-pointer hover:bg-gray-50 border-b border-gray-50 last:border-0 ${
                opt.value === value ? "bg-[#c62d23]/5 font-semibold text-[#c62d23]" : "text-gray-800"
              }`}
            >
              {opt.label}
            </div>
          ...
          {allowNew && (
            <div
              ...
              className="px-4 py-2.5 bg-gray-50 hover:bg-green-50 cursor-pointer text-green-700 font-semibold text-sm border-t border-gray-100 flex items-center gap-1.5"
            >
              <Plus size={14} />
              {newLabel || "Add new…"}
            </div>
...
/* ─────────────────────────────────────────────────────────
   MINI INLINE FORM  (add new item / new location)
───────────────────────────────────────────────────────── */
function InlineAddForm({ label, placeholder, onConfirm, onCancel }) {
  const [val, setVal] = useState("");
  return (
    <div className="flex items-center gap-2 bg-green-50 border border-green-200 rounded-xl px-3 py-2 mt-1">
      <input
        autoFocus
        value={val}
      ...
      />
      <button
        onMouseDown={() => { if (val.trim()) onConfirm(val.trim()); }}
        className="text-xs font-bold text-green-700 hover:text-green-800 shrink-0 px-2 py-1 rounded-lg hover:bg-green-100 transition-colors"
      >
        Add
      </button>
      <button
...
export default function StockMovementsPage() {
  ...
  // New-item / new-location inline-add state
  const [addingNewItemForRow, setAddingNewItemForRow] = useState(null); // row index or null
  const [addingNewLocation, setAddingNewLocation] = useState(false);
  const [customItems, setCustomItems] = useState([]);   // [{label,value}] temp new parts
  const [customLocations, setCustomLocations] = useState([]); // extra locations typed by user
  ...
  useEffect(() => { fetchMovements(); fetchInventory(); }, []);
  ...
  const removeRow = (index) => setSelectedItems((prev) => prev.filter((_, i) => i !== index));
  ...
  const productOptions = useMemo(() => {
    const fromInventory = inventoryList
      ...
      .map((item) => ({
        label: `${item.partName || item.chairType} — ${item.location}`,
        value: item._id,
      }));
    return [...fromInventory, ...customItems];
  }, [inventoryList, customItems]);
  ...
  const locationOptions = useMemo(() => {
    ...
    return base.map((l) => ({ label: l, value: l }));
  }, [inventoryList, customLocations]);
  ...
  const handleTransfer = async () => {
    ...
    try {
      ...
      for (const item of selectedItems) {
        ...
        // If it's a custom (non-inventory) item, skip stock checks
        if (sourceItem) {
  ...
  /* ── NORMALIZE ── */
  const normalized = useMemo(() => movements.map((m) => ({
    id: m._id,
  ...
  const filtered = useMemo(() => {
    ...
    if (activeStatFilter === "TODAY") {
      data = data.filter((m) => new Date(m.time).toDateString() === new Date().toDateString());
    }
    return data;
  ...
  const todayCount = normalized.filter((m) => new Date(m.time).toDateString() === new Date().toDateString()).length;
    ...
    <div className="flex h-screen bg-[#f8f7f5] text-gray-900">
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden">
        ...
        <div className="lg:hidden sticky top-0 z-30 bg-white border-b border-gray-100 px-4 py-3 flex items-center justify-between shadow-sm">
          ...
          <button onClick={() => setTransferModalOpen(true)} className="bg-[#c62d23] hover:bg-[#a8241c] text-white p-2 rounded-lg transition-colors active:scale-95">
        ...
        <div className="px-4 sm:px-6 lg:px-8 py-5 space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <StatCard title="Total Movements" value={totalMovements} icon={<ArrowRightLeft />} accentColor="#c62d23" clickable active={activeStatFilter === "ALL"} onClick={() => setActiveStatFilter("ALL")} />
            <StatCard title="Total Qty Moved" value={totalQty} icon={<TrendingUp />} accentColor="#3b82f6" clickable={false} />
            <StatCard title="Today's Movements" value={todayCount} icon={<Clock />} accentColor="#f59e0b" clickable active={activeStatFilter === "TODAY"} onClick={() => setActiveStatFilter("TODAY")} />
          </div>
          ...
          {activeStatFilter === "TODAY" && (
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 flex items-center gap-2.5">
              ...
              <button onClick={() => setActiveStatFilter("ALL")} className="ml-auto text-xs font-semibold text-amber-700 hover:text-amber-900 underline underline-offset-2">Show all</button>
            </div>
          )}
          ...
          <div className="flex flex-col sm:flex-row gap-3 flex-wrap">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search part, location or user…" className="w-full bg-white border border-gray-200 pl-10 pr-4 py-2.5 rounded-xl text-sm outline-none text-gray-900 placeholder-gray-400 focus:border-[#c62d23] focus:ring-2 focus:ring-[#c62d23]/15 transition-all shadow-sm" />
            </div>
            <select value={partFilter} onChange={(e) => setPartFilter(e.target.value)} className="bg-white border border-gray-200 px-3 py-2.5 rounded-xl text-sm outline-none text-gray-700 focus:border-[#c62d23] transition-all shadow-sm min-w-[150px]">
              <option value="All">Part (All)</option>
              {partNames.map((p) => <option key={p} value={p}>{p}</option>)}
            </select>
            <select value={fromFilter} onChange={(e) => setFromFilter(e.target.value)} className="bg-white border border-gray-200 px-3 py-2.5 rounded-xl text-sm outline-none text-gray-700 focus:border-[#c62d23] transition-all shadow-sm min-w-[140px]">
              <option value="All">From (All)</option>
              {locations.map((l) => <option key={l} value={l}>{l}</option>)}
            </select>
            <select value={toFilter} onChange={(e) => setToFilter(e.target.value)} className="bg-white border border-gray-200 px-3 py-2.5 rounded-xl text-sm outline-none text-gray-700 focus:border-[#c62d23] transition-all shadow-sm min-w-[140px]">
              <option value="All">To (All)</option>
              {locations.map((l) => <option key={l} value={l}>{l}</option>)}
            </select>
            {hasActiveFilters && (
              <button onClick={() => { setSearch(""); setPartFilter("All"); setFromFilter("All"); setToFilter("All"); setActiveStatFilter("ALL"); }} className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-sm font-medium bg-white border border-gray-200 text-gray-600 hover:border-[#c62d23] hover:text-[#c62d23] transition-all shadow-sm shrink-0">
                <X size={14} />Reset
              </button>
            )}
          </div>
              ...
              <>
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {paginatedRows.map((m) => (
                        <tr key={m.id} className="hover:bg-gray-50/80 transition-colors group">
                          ...
                          <td className="px-4 py-3.5 whitespace-nowrap">
                            <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-[#c62d23]/10 text-[#c62d23] border border-[#c62d23]/20">{m.qty}</span>
                          </td>
                          <td className="px-4 py-3.5 whitespace-nowrap">
                            <div className="flex items-center gap-1.5">
                  ...
                  {paginatedRows.map((m) => (
                    <div key={m.id} className="p-4 bg-white">
                      <div className="flex items-start justify-between gap-2 mb-3">
                        ...
                        <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-[#c62d23]/10 text-[#c62d23] border border-[#c62d23]/20 shrink-0">{m.qty}</span>
                      </div>
                      ...
                      <div className="flex items-center justify-between text-xs text-gray-500">
                        <div className="flex items-center gap-1.5">
                          <div className="w-5 h-5 rounded-full bg-gray-100 flex items-center justify-center text-[9px] font-bold text-gray-600">{m.user.charAt(0).toUpperCase()}</div>
                          <span>{m.user}</span>
                        </div>
                ...
                <div className="px-4 py-3 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-1 order-1 sm:order-2">
                    <button onClick={() => setCurrentPage((p) => Math.max(1, p - 1))} disabled={currentPage === 1} className="px-3 py-1.5 text-xs rounded-lg border border-gray-200 font-medium transition-colors disabled:opacity-40 disabled:cursor-not-allowed hover:border-[#c62d23] hover:text-[#c62d23]">← Prev</button>
                    {Array.from({ length: totalPages }, (_, i) => i + 1)
                      ...
                      .map((p, idx) =>
                        p === "..." ? (
                          ...
                          <button key={p} onClick={() => setCurrentPage(p)} className={`w-8 h-8 text-xs rounded-lg border font-semibold transition-colors ${currentPage === p ? "bg-[#c62d23] text-white border-[#c62d23]" : "border-gray-200 text-gray-600 hover:border-[#c62d23] hover:text-[#c62d23]"}`}>{p}</button>
                        )
                      )}
                    <button onClick={() => setCurrentPage((p) => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="px-3 py-1.5 text-xs rounded-lg border border-gray-200 font-medium transition-colors disabled:opacity-40 disabled:cursor-not-allowed hover:border-[#c62d23] hover:text-[#c62d23]">Next →</button>
                  </div>
                </div>
              </>
            )}
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-xl max-h-[92vh] flex flex-col shadow-2xl border border-gray-100">
            ...
            <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-gray-100">
              ...
              <button onClick={() => { setTransferModalOpen(false); setCustomItems([]); setCustomLocations([]); setAddingNewItemForRow(null); setAddingNewLocation(false); }} className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition-colors">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="px-6 py-5 overflow-y-auto flex-1 space-y-5">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wide">Products</label>
                  <span className="text-xs text-gray-400">{selectedItems.length} item{selectedItems.length !== 1 ? "s" : ""}</span>
                </div>

                {/* Column headers */}
                <div className="grid grid-cols-[1fr_80px_80px_32px] gap-2 px-1 mb-1">
                  ...
                  {selectedItems.map((row, index) => {
                    ...
                    return (
                      <div key={index} className="space-y-1">
                        <div className="grid grid-cols-[1fr_80px_80px_32px] gap-2 items-center bg-gray-50 rounded-xl px-3 py-2.5 border border-gray-200">
                          {/* Product combobox */}
                          <SearchableCombobox
                            value={row.sourceId}
                            onChange={(val) => updateItem(index, "sourceId", val)}
                            options={productOptions}
                            placeholder="Select Product"
                            allowNew
                            newLabel="Add new item"
                            onAddNew={() => setAddingNewItemForRow(index)}
                          />
                          ...
                          <div className="flex justify-center">
                            {selectedItems.length > 1 ? (
                              <button onClick={() => removeRow(index)} className="p-1 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors">
                                <X size={14} />
                              </button>
                            ) : <div className="w-6" />}
                          </div>
                        ...
                        {/* Inline add-new-item form for this row */}
                        {addingNewItemForRow === index && (
                          <InlineAddForm
                            label="New item name"
                            placeholder="e.g. Armrest Pad, Gas Cylinder…"
                            onConfirm={(name) => {
                              const newId = `custom_${Date.now()}`;
                              setCustomItems((prev) => [...prev, { label: name, value: newId }]);
                              updateItem(index, "sourceId", newId);
                              setAddingNewItemForRow(null);
                            }}
                            onCancel={() => setAddingNewItemForRow(null)}
                          />
                        )}
                      </div>
                    );
              ...
              <div>
                ...
                <SearchableCombobox
                  ...
                  onAddNew={() => setAddingNewLocation(true)}
                />

                {addingNewLocation && (
                  <InlineAddForm
                    placeholder="e.g. STORE_B, Assembly Line 3…"
                    onConfirm={(loc) => {
            ...
            <div className="flex justify-end gap-2 px-6 py-4 border-t border-gray-100">
              <button
                type="button"
                onClick={() => { setTransferModalOpen(false); setCustomItems([]); setCustomLocations([]); setAddingNewItemForRow(null); setAddingNewLocation(false); }}
...
function StatCard({ title, value, icon, accentColor, clickable, active, onClick }) {
  return (
    <div
      ...
      className={`bg-white border rounded-xl p-4 sm:p-5 transition-all duration-200 shadow-sm hover:shadow-md group ${clickable ? "cursor-pointer" : ""} ${active ? "ring-2 ring-offset-1" : "hover:border-gray-200 border-gray-100"}`}
